...
//! # 跨架构系统调用 ABI 描述和包装器
//! 
//! 通过填写结构体字段，描述 x86_64、aarch64 和 riscv64 在 Linux 上的系统调用 ABI。
...
//! 
//! | 架构     | 指令       | 系统调用 ID 寄存器 | 返回值寄存器 | 参数寄存器                      |
//! |----------|------------|------------------|-------------|--------------------------------|
//! | x86_64   | `syscall`  | rax              | rax         | rdi, rsi, rdx, r10, r8, r9     |
//! | aarch64  | `svc #0`   | x8               | x0          | x0, x1, x2, x3, x4, x5        |
//! | riscv64  | `ecall`    | a7               | a0          | a0, a1, a2, a3, a4, a5         |
...
//! ## 任务
//! 
//! 1. 实现 `x86_64_abi()`、`aarch64_abi()`、`riscv64_abi()` — 返回描述每个架构 ABI 的结构体
//! 2.（条件编译）在当前平台上实现真实的 `syscall3` 内联汇编
//! 3. 在 `syscall3` 基础上构建 `sys_write` / `sys_read` / `sys_close` / `sys_exit`
//! 
//! ## 提示
//! 
//! - Linux 系统调用号在不同架构之间有所不同；x86_64 与 aarch64/riscv64 有很大差异
//! - x86_64 的 `syscall` 指令会修改 rcx 和 r11 寄存器
