...
//! # RAII 自旋锁守卫
//! 
//! 在本练习中，你将为一个自旋锁实现 RAII 守卫，使锁在离开作用域时自动释放。
//! 这是 Rust 所有权系统在系统编程中的经典应用。
//! 
//! ## 关键点
//! - RAII（资源获取即初始化）模式
//! - 用于透明访问的 `Deref` / `DerefMut` 特质
//! - 用于自动释放的 `Drop` 特质
//! - 为什么手动 lock/unlock 是不安全的（忘记解锁、panic 时未释放）
...
// Return &mut T
//
// TODO: 为 SpinGuard 实现 DerefMut 特质
// 返回 &mut T
