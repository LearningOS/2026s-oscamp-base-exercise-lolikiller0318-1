...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        // TODO: 实现 TLB + 页表的二级查找
        //todo!()
        let ppn = self.tlb.lookup(vpn, self.current_asid);
        if let Some(ppn) = ppn {
            return Some(ppn);
        } else if let Some((asid, mapping)) = self.page_table.iter().find(|(asid, mapping)| *asid == self.current_asid && mapping.vpn == vpn) {
